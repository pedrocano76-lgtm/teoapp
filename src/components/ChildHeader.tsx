import { Child } from '@/lib/types';
import { getAge } from '@/lib/age-utils';
import { cn } from '@/lib/utils';

interface ChildHeaderProps {
  child: Child;
  photoCount: number;
}

const avatarMap: Record<Child['color'], string> = {
  primary: 'bg-primary',
  sage: 'bg-sage',
  lavender: 'bg-lavender',
  peach: 'bg-peach',
  sky: 'bg-sky',
};

export function ChildHeader({ child, photoCount }: ChildHeaderProps) {
  return (
    <div className="flex items-center gap-2 py-2 mb-3 border-b border-border">
      <div className={cn(
        'w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-primary-foreground shrink-0',
        avatarMap[child.color]
      )}>
        {child.name[0]}
      </div>
      <span className="font-medium text-foreground text-sm">{child.name}</span>
      <span className="text-xs text-muted-foreground truncate">
        · {getAge(child.birthDate)} · {photoCount} {photoCount === 1 ? 'foto' : 'fotos'}
      </span>
    </div>
  );
}
